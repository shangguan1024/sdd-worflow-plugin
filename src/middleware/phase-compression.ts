...
export const phaseCompressionMiddleware = {
  check(
    ...
    if (phase <= 0 || !state.featureName) return { allowed: true }
    ...
    if (!content.toLowerCase().includes(spec.marker.toLowerCase())) {
      ...
      const found = alts.some(
        (alt) => content.toLowerCase().includes(alt.toLowerCase())
      )
      if (!found) {
        return {
          allowed: false,
          ...
          suggestion:
            "Include: what completed, key decisions, issues, pending, next phase notes.",
        }
      }
    }
