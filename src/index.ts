...
const SddWorkflowPlugin: Plugin = async (
  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      const artifactResult = artifactCheckMiddleware.check(
        phase,
        state,
        directory
      )
      ...
      const loopResult = loopDetectionMiddleware.afterEdit(
        toolName,
        output.args ?? {},
        state
      )
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (contextMonitorMiddleware.shouldRefresh(state)) {
        const refreshContent = contextMonitorMiddleware.generateRefresh(
          state,
          directory
        )
        output.metadata = {
    ...
    "experimental.chat.system.transform": async (_input, output) => {
      ...
      output.system.push(
        `## SDD-Workflow (Phase ${phase}: ${state.getPhaseName()})\n\n${phasePrompt}\n\n${memoryContext}`
      )
    },
