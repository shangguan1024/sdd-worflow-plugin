...
import { Phase, PHASE_NAMES, SddState } from "../state.js"
...
export class RollbackHandler {
  ...
  executeRollback(targetPhase: number, codeScope: string, confirmed: boolean): RollbackResult {
    if (!confirmed) {
      ...
      if (plan.otherCode.length > 0) {
        details.push(
          "",
          "⚠️ Other code changes (NOT in task_plan.md) — requires YOUR decision:",
          ...plan.otherCode.map(f => `  - ${f}`),
          "",
          "code_scope options:",
          "  'none'      : Skip ALL code rollback (SDD state + docs only)",
          "  'related'   : Auto rollback related code, list other code for your choice",
          "  'all'       : Rollback everything (including other code)",
          "",
          "Set confirmed=true + choose code_scope to execute.",
        )
      } else {
        details.push("", "No unrelated code changes detected.", "Set confirmed=true to execute rollback.")
      }

      if (plan.gitShaAtTarget) details.push("", `Git SHA at target phase: ${plan.gitShaAtTarget}`)
      if (plan.currentSha) details.push(`Current HEAD: ${plan.currentSha}`)
    ...
    if (plan.sddDocs.length > 0) {
      ...
      for (const f of plan.sddDocs) results.push(`  - ${f}`)
    } else {
    ...
    if (codeScope === "none") {
      ...
      if (plan.relatedCode.length > 0) {
        ...
        for (const f of plan.relatedCode) results.push(`  - ${f}`)
      } else {
      ...
      if (plan.otherCode.length > 0) {
        if (codeScope === "all") {
          ...
          for (const f of plan.otherCode) results.push(`  - ${f}`)
        } else {
          results.push("⚠️ Other code NOT rolled back (user must decide per file):")
          for (const f of plan.otherCode) results.push(`  - ${f}`)
          results.push("", "To rollback individually:")
          for (const f of plan.otherCode) results.push(`  git checkout ${plan.gitShaAtTarget ?? "HEAD~1"} -- "${f}"`)
  ...
  private findShaAtPhase(targetPhase: number): string | undefined {
    ...
    for (let phase = targetPhase; phase >= 0; phase--) {
      ...
      if (existsSync(phaseCheckpoint)) {
        try {
          ...
          if (data.gitSha) return data.gitSha as string
        } catch { continue }
      }
    }
    ...
    if (existsSync(mainCheckpoint)) {
      try {
        ...
        if (cpPhase <= targetPhase && data.gitSha) return data.gitSha as string
      } catch { /* fall through */ }
    }

    try {
      const log = execSync(
        `git log --oneline -- "docs/features/${this.state.featureName}/"`,
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        const hasDesignOrPlan = files.some(f =>
          f.includes("design.md") || f.includes("task_plan.md") || f.includes("findings.md")
        )
        if (!hasDesignOrPlan) return sha
      }
  ...
  private getFeatureChangedFiles(featureDir: string): string[] {
    try {
  ...
  private getTaskPlanFiles(featureDir: string): string[] {
    ...
    try {
      ...
      while ((m = tagPattern.exec(content)) !== null) files.push(m[1])
  ...
  private gitRestore(files: string[], sha?: string): boolean {
    ...
    try {
      ...
      for (const f of files) {
        try { execSync(`git checkout ${target} -- "${f}"`, { cwd: this.projectDir, encoding: "utf-8" }) }
  ...
  private writeCheckpoint(targetPhase: number): void {
    const checkpointDir = join(this.projectDir, "docs", "features", this.state.featureName, ".sdd")
