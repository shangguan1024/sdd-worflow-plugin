...
export function toolDefinitions(director, state) {
    ...
    return {
        ...
        sdd_start: tool({
            description: "Start a new feature development workflow (Phase 0: Research & Understanding)",
            args: {
        ...
        sdd_resume: tool({
            ...
            args: {
                feature: tool
                    ...
                    .string()
                    .optional()
                    .describe("Feature name to resume (optional, uses current if not specified)"),
            },
            async execute(args) {
        ...
        sdd_status: tool({
            description: "Show current SDD workflow status: phase, feature, edit counts, hot files",
            args: {
                feature: tool
                    .schema
                    .string()
        ...
        sdd_complete: tool({
            description: "Force complete the current workflow (Phase 6: Memory Persistence)",
            args: {
        ...
        sdd_gate: tool({
            description: "Phase gate operations: check requirements, approve transition, or block. Use before transitioning between phases. APPROVE requires human confirmation.",
            args: {
                phase: tool
                    .schema
                    .number()
                ...
                action: tool
                    ...
                    .describe("check=show requirements, approve=request human confirmation then transition, block=force stay in current phase"),
                confirmed: tool
                    .schema
                    .boolean()
                    .optional()
            ...
            async execute(args) {
                ...
                if (action === "approve") {
                    ...
                    if (!success) {
                        return formatResult({
                            success: false,
                            message: `Cannot transition to Phase ${phase}. Current: Phase ${state.currentPhase} (${state.getPhaseName()}). Gate not passed or invalid transition.`,
                        });
                    }
                    return formatResult({
                        success: true,
                        message: `✅ Gate approved by human. Transitioned to Phase ${phase} (${state.getPhaseName()}).`,
                    });
                }
                if (action === "block") {
                    return formatResult({
        ...
        sdd_refresh: tool({
            description: "Force context refresh - inject key requirements, design decisions, and hot file warnings to prevent context drift",
            args: {
        ...
        sdd_memory_timeline: tool({
            description: "Get memory timeline around a specific node (Layer 2: Progressive Disclosure). Shows recent decisions, requirements, and context.",
            args: {
                around_id: tool
                    .schema
                    .string()
        ...
        sdd_memory_details: tool({
            description: "Get full details for specific memory nodes (Layer 3: Full Disclosure). Complete content, alternatives, rationale.",
            args: {
